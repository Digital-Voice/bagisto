<div class="flex flex-wrap gap-1.5">
    @php
        $restCount = max($cart->items->count() - 3, 0);
    @endphp

    @foreach ($cart->items->take(3) as $item)
        <div class="relative">
            <a target="_blank" title="{{ $item->product->name }}" href="{{ route('shop.product_or_category.index', $item->product->url_key) }}">
    ...
    @if ($restCount >= 1)
        <a href="{{ route('admin.sales.carts.view', $cart->id) }}">
